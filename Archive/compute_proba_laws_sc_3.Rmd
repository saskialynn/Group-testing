...
for (N in 1:pool.max){
  for (prev in c(0.005, 0.01, 0.02)){
    ...
    for(tau in c("house", "meal", "travel")){
      print(c(prev,tau, N))
...
pdf(file.path(path.out.figures, "sensitivity_plots_taucat.pdf"), width=8, height=5)
sens_plots
dev.off()
...
## HOUSE only
sens_plots <- lapply(types[2:5], function(i){
  ggplot(conf_int %>% filter(type == i & tau == "house"), aes(x=pool_size, y= mean_sens)) +
  geom_line(aes(color=as.factor(tau), linetype = "Alternative Model")) +
  geom_line(data=conf_int[conf_int$type == "Null Model",], aes(linetype = "Null Model"))+
  facet_wrap(~ prev) +
  geom_ribbon(data = . %>% filter(type==i), aes(x=pool_size, ymin = sens_q025, ymax = sens_q975, 
...
pdf(file.path(path.out.figures, "sensitivity_plots_taucat_house.pdf"), width=8, height=5)
sens_plots
dev.off()
```
