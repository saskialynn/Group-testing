...
experiment_loop <- function(above, ct_dat, sim_min, sim_max){
  ...
  rbindlist(lapply(1:pool.max, function (p) { # pool size
    rbindlist(lapply(c(0.001,0.01, 0.03, 0.05,0.1), function(prevalence) { # prevalence ("proportion positive tests")
      ...
      rbindlist(lapply(c(0, 0.1, 0.2, 0.3, 0.4), function(tau) {
        ...
        rbindlist(lapply(0:p, function(positives) { # number of positives
           ...
           if (positives == 0) {
            ...
            data.frame(
              ...
              # random=sample(n,n,replace = TRUE)/n, # n-dim vector
              random = runif(n, 0, 1),
              ...
              # get rid of call.each.conc
              mutate(
                call.each.conc=probit[1+(z.index-1)*571+each.conc*10-(lod-35.9)*10,2]>random, 
                # tp = probit, fn = 1-probit (z.index is the probit score)
                # repeat n=10000 times checking whether > random
                # for each combination of variables, we have 10,000 experiments
...
allfirst.poolct <- rbindlist(lapply(1:2, experiment_loop, ct_dat = ct_fake_input)) # just for 15% Ct > LoD 
...
null_res <- allci[,c(1:6, 8, 10, 12, 18:22, 24:25)]
null_res$type <- "null"
alt_res <- allci[,c(1:6, 7, 9, 11, 13:17, 24:25)]
alt_res$type <- "alt"
names(null_res) <- names(alt_res)
allci_restruct <- rbind(null_res, alt_res)

# allci <- subset(allci, tau == 0.1)
ggplot(data = subset(allci, tau == 0.1), mapping = aes(x=pool, y= ppa, color=as.factor(prevalence))) +
  geom_line() +
  geom_ribbon(aes(ymin=lwr.ci, ymax=upr.ci), linetype=2, alpha=0.1)
...
allci_tauconst <- read.csv("correlated_exactprobs_Ct15.csv")
summary_table <- allci %>%
...
experiment_loop <- function(above, ct_dat, sim_min, sim_max){
  ...
  threshold.ct <- c(sample(ct_set, n, replace=T)) # sample ct_set with replacement
  rbindlist(lapply(1:pool.max, function (p) { # pool size
    num_pools <- ceiling(group.max / p)
    rbindlist(lapply(c(0.001,0.01, 0.03, 0.05,0.1), function(prevalence) { # prevalence ("proportion positive tests")
      # sim_probs <- calc_probs_all_homogeneous(p, prevalence, min=sim_min, max=sim_max, B=1000)
      # specify range of correlation to sample from (min, max)
      rbindlist(lapply(c(0, 0.1, 0.2, 0.3, 0.4), function(tau) {
        #sim_probs <- exact_probs(group_size = p, tau = tau, pop_prev = prevalence)
        sim_probs <- exact_probs(group_size = p, tau = tau, pop_prev = prevalence, tau_sample = TRUE, num_pools = num_pools)
        # sim_probs <- exact_probs(group_size = 1, tau = .2, pop_prev = .1)
        rbindlist(lapply(0:p, function(positives) { # number of positives
...
# write.csv(allci, "correlated_exactprobs_tauhet_Ct15.csv")
# write.csv(allci, "correlated_exactprobs_Ct15.csv") 
# write.csv(allci,"correlated_individuals_rho.1.2.csv")
# write.csv(allci,"correlated_individuals_rho.4.6.csv")
...
```{r}
allci <- read.csv("correlated_exactprobs_tauhet_Ct15.csv")
ppa_plot <- ggplot() +
  geom_line(allci, mapping = aes(x=pool, y= ppa, color=as.factor(prevalence),linetype="correlated")) +
  geom_line(allci, mapping = aes(x=pool, y=ppa_null, color=as.factor(prevalence),linetype="null")) +
...
library(ggpubr)
pdf("plot.ppa.eff.corr.tauhet.ct15.pdf",width=8, height=5)  
ggarrange(ppa_plot, tests_plot, 
          labels = c("A", "B"), 
          ncol=1, nrow = 2, 
          common.legend = TRUE, legend = "right")
...
```{r}
test <- read.csv("correlated_exactprobs_Ct15.csv")

allci[allci$pool == 17,]$ppa_null
test[test$pool == 17, ]$ppa_null
```
