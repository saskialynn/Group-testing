...
experiment_loop <- function(above, ct_dat){
  ...
  rbindlist(lapply(2:pool.max, function (p) { # pool size
    rbindlist(lapply(c(0.001,0.01,0.05,0.1), function(prevalence) { # proportion positive tests 
      
      sim_probs = calc_probs(p, prevalence, min=0.6, max=0.8, B=1000)
      rbindlist(lapply(0:p, function(positives) { # number of positives
        ...
        if (positives == 0) {
          data.frame(limit=lod, pool=p, pos=positives, prevalence=prevalence, 
                     ...
                     probability_null = dbinom(positives, p, prevalence), random=0, z.index=0,
...
#allfirst.poolct <- experiment_loop(above=1, ct_dat = ct_fake_input)
allfirst.poolct2 = group_by(allfirst.poolct, pool, prevalence, above.llod, limit, pos) %>%
  summarise_all(mean)

group_by(allfirst.poolct2, pool, prevalence, above.llod, limit) %>% 
...
write.csv(allci,"test.csv")
```

```{r}
ggplot(all, aes(x=pool, y= tp1/(tp1 + fn1), color=as.factor(prevalence))) + 
  geom_line() +
...
```{r}
ggplot(all, aes(x=pool, y=tp_null/(tp_null + fn_null), color=as.factor(prevalence))) + 
  geom_line() +
  facet_wrap(vars(factor(above.llod, labels=c("5% samples Ct>LoD","10% samples Ct>LoD","15% samples Ct>LoD","20% samples Ct>LoD","25% samples Ct>LoD","30% samples Ct>LoD")))) +
  theme_bw() + 
  scale_color_brewer(palette="RdYlBu", name="Prevalence") + 
...
```
```{r}
ggplot(all, aes(x=pool, y=fn1, color=as.factor(prevalence))) + 
  geom_line() +
  geom_line(aes(x=pool, y=fn_null, color=as.factor(prevalence), linestyle="dashed"))+
  facet_wrap(vars(factor(above.llod, labels=c("5% samples Ct>LoD","10% samples Ct>LoD","15% samples Ct>LoD","20% samples Ct>LoD","25% samples Ct>LoD","30% samples Ct>LoD")))) +
  theme_bw() + 
  scale_color_brewer(palette="RdYlBu", name="Prevalence") + 
  xlab("Pool size") + ylab("Expected positive percent agreement") +
