    ...
    rbindlist(lapply(c(0.001,0.01,0.03,0.05,0.1,0.15), function(prevalence) { # proportion positive tests 
      rbindlist(lapply(2:p, function(positives) { # number of positives
        ...
        if (positives == 0) {
          ...
          t <- sapply(sample(threshold.ct$Z, n, replace = T), function(x) return(threshold.ct$Ct[which(threshold.ct$Z == x)[1:min(frac_pos, positives)]])) # for each experiment, sample one group
          ...
          # t_dat <- ifelse(positives == 1, t(as.matrix(t)), as.matrix(t))
