...
>>>>>>> dd02b2908eecbbcbed04771ab0b49898c7d8da09
}
...
>>>>>>> dd02b2908eecbbcbed04771ab0b49898c7d8da09
  ...
  if (is.null(tau_subject_effect) == FALSE){
    taus_f <- function(b){tau_subject_effect(x=tau)}
    dist_taus = lapply(1:B, function(b){ #for B simulations
      sapply(1:N, function(K){ 
        sapply(1:(K), function(k){
          pp = mean(sapply(1:B, function(x){1- exp(sum(log(1-tau_subject_effect(x=tau, k))))}))
          choose(N-k, K-k)  * pp^(K-k) * (1-pp)^(N-K)
        })
      })
      # sapply(1:N, function(K){ 
      #   #### generate subject effects
...
sens_fxn <- function(N){ # change this so ct_dat, n, B, etc are not implicit arguments (pass to fxn)
  sapply(1:N, function(positives){
    ...
    each.conc = -log2(colSums(2^-dat)/pool.size)+ifelse(dilution.vary.index==1,0, # pool.size should be N? 
...
```{r}
ggplot(probit %>% group_by(ct_value) %>% summarize(probit_probability_detection = mean(probit_probability_detection)), aes(x=ct_value, y=probit_probability_detection)) + 
  geom_point()
```
