        ...
        rbindlist(lapply(0:p, function(positives) { # number of positives
           ...
           if (positives == 0) {
            ...
            data.frame(limit=lod, pool=p, pos=positives, prevalence=prevalence, 
                       ...
                       probability_null = dbinom(positives, n_eff, prev_eff), # prevalence corrected for correlated individuals
                       random=0, z.index=0,
            ...
            data.frame(
              ...
              prevalence_corr = prev_eff,
              probability_null = dbinom(positives, n_eff, prev_eff)/n,
...
group_by(allfirst.poolct, pool, prevalence, above.llod, limit, tau) %>% 
  summarize(pos1=weighted.mean(pos, w=probability),
            ...
            ppa_subset = weighted.mean(tp/(fn + tp), w=probability_subset1, na.rm = TRUE),
            tn_subset =weighted.mean(tn, w=probability_subset1), 
            tp_subset =weighted.mean(tp, w=probability_subset1), 
            fn_subset =weighted.mean(fn, w=probability_subset1), 
            fp_subset =weighted.mean(fp, w=probability_subset1)) -> apw
# rm(allfirst.poolct)
