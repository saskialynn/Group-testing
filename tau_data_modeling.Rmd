...
ggplot(all_SAR, aes(x=num_contacts)) + 
  geom_histogram()
```
```{r}
...
hist(theta_i)
hist(all_SAR$SAR, alpha=0.5,add=TRUE, col="green")

ggplot(all_SAR)+
  geom_histogram(aes(x=theta_i), alpha=0.3, fill="red", position="dodge")+
  geom_histogram(aes(x=SAR), alpha=0.3, fill="green", position="dodge")

hist(dbinom(x=all_SAR$secondary, size=all_SAR$num_contacts, prob=theta_i), 
     breaks = 20)
```

Simulate number of infections using fited distribution, compare to observed
```{r}
B <- 1000
