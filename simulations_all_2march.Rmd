...
pool.max = 20
prev_input = c(0.001,0.01, 0.03, 0.05,0.1)
tau_input = c(0.1, 0.2, 0.3, 0.4)
pool_input = seq(1, pool.max, by = 1)
mode = "uniform" # generating mechanism for taus
mode_prev = "uniform" # generating mechanism for prevalence
# filename = args[6]
```
...
pool_input, prev_input, tau_input
  ...
  A = rbindlist(lapply(pools, function (pool.size) { # pool size
    rbindlist(lapply(prevalences, function(prev) { # prevalences
      rbindlist(lapply(taus, function(tau) { # taus
        rbindlist(lapply(c(1, 1.1, 1.2, 1.5, 2, 2.5, 3,4,5,6,7,10), function(tau_relative_var) { # alpha value
          # print(c(pool.size, prev, tau, tau_relative_var, mode, mode_prev)) 
          
          prev <- prev_input[1]
          pool.size <- pool_input[1]
          tau <- tau_input[1]
          tau_relative_var <- 1
...
```{r}
k <- (pool.size - 2)*1
alpha <- tau_relative_var
typeof(k)
typeof(alpha)
rnorm(k, log(tau /(1-tau)), sd = log(alpha)/2)
rnorm(-1, -2.19, 0)
```
