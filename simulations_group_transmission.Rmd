...
# fxn to calculate p(k positives | group size, prevalence)
calc_probs <- function(N){ # N = group size, prev = prevalence
  # create nodes
  nodes <- tibble(id = 1:N)
  ...
  Sigma <- matrix(runif(N^2, .4, .6), nrow = N, ncol = N) # generate random values from .1 to 0.9
  Sigma = 0.5 * (Sigma + t(Sigma)) # make matrix symmetrical 
  diag(Sigma)= rep(1,N) # 1's on diagonal 
...
sim_probs_house <- matrix(unlist(lapply(2:20, calc_probs)), nrow = (group_max), ncol = (group_max-1))
