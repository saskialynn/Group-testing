...
v_long <- rep(1,N) #rep(1:10, each = 2)
v <- v_long[1:N]
prev <- 0.01
risk_probs <- prev*v
...
```{r}
group_max <- 20 # maximum group size
B <- 100000 # number of repetitions of simulation
...
```{r}
sim_probs_house <- matrix(unlist(lapply(2:20, calc_probs, prev = 0.01, min = 0.4, max= 0.6)), nrow = (group_max), ncol = (group_max-1))
...
```{r}
sim_probs_.1.2 <- matrix(unlist(lapply(2:20, calc_probs, prev, min = 0.1, max= 0.2)),
                         nrow = (group_max),
                         ncol = (group_max-1))
...
ggplot() +
  geom_line(probs_dat, mapping = aes(x=pool, y=prob_sim, color=as.factor(pos))) +
