...
ct_fake_input <- data.frame("Ct" =create_correlated_Cts(G),
                            "Z" = rep(1,G))
for (i in 2:(N%/%G)){
  ct_fake_input <- rbind(Ct_fake_input,
                         data.frame("Ct" =create_correlated_Cts(G),
                                     "Z" = rep(i,G))
  )
}
...
```{r probit_model}
# probit data input
probit_input <- read.csv("~/Dropbox/Group-testing/probit_zscores_cts_tissue_agnostic.csv")
...
allfirst.poolct <- data.table::rbindlist(lapply(1:length(above.lod),
                                                function(above){
