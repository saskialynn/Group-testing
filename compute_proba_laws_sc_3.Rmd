...
sens_fxn <- function(N, B = 1000, above = 3, ct_dat = ct_fake_input){ 
  set.seed(123)
  ...
  sapply(1:N, function(positives){
    ...
    each.conc = -log2(colSums(2^-dat)/N)+ifelse(dilution.vary.index==1,0, 
    ...
    # TODO: ask Hannah what this is
    # want ct values where z.index = 0 
    # merge(probit %>% filter(z_value == 0), each.)
...
temp <- df %>% summarize(sum(df$probit_probability_detection * freq_ct_dat$freq))
```
...
sens_dat <- data.frame(pool.size = 1, 
             positives = 1, 
             sensitivity = sens_fxn(1))
for (p in 2:20){
 sens_dat <- rbind(sens_dat, data.frame(pool.size=p, 
           positives = c(1:p), 
           sensitivity = sens_fxn(p))) 
}

sens_dat$pos_frac <- sens_dat$positives / sens_dat$pool.size


ggplot(sens_dat, aes(x=pool.size, y=sensitivity, group=as.factor(positives))) +
  geom_line(aes(color=as.factor(positives))) + 
  theme(legend.position="bottom") + 
  guides(fill=guide_legend(title="Number Positive")) + 
  geom_hline(yintercept = sens_dat$sensitivity[sens_dat$pool.size==1]) + 
  xlab("Pool size") + ylab("Sensitivity") 

ggplot(sens_dat, aes(x=pos_frac, y=sensitivity, group=as.factor(pool.size))) +
...
```
TODO: plot (1) sensitivity as a function of Ct values
```{r}
# make a new sensitivity fxn that has output useful for plotting 
sens_fxn2 <- function(N, B = 1000, above = 3, ct_dat = ct_fake_input){ 
  set.seed(123)
  ct_set <- pmax(5,subset(ct_dat + mat2[above,3], ct_dat + mat2[above,3]<45)) 
  l <- sapply(1:N, function(positives){
    ...
    each.conc = -log2(colSums(2^-dat)/N)+ifelse(dilution.vary.index==1,0, 
                                                rnorm(mean=0,sd=1.1,n=ncol(dat)))
    z.index= probit.z.indices[probit.mode.index]
    sensitivity = mean(probit[1+(z.index-1)*571+each.conc*10-(lod-35.9)*10,2])
    ct_avg = mean(each.conc)
    cbind(N, positives, sensitivity, ct_avg)
  }) 
  t <- t(l)
  colnames(t) <- c("pool.size", "positives", "sensitivity", "ct_avg")
  return(t)
} 

sens_dat2 <- data.frame(do.call(rbind, sapply(1:20, function(n) { sens_fxn2(n)})))
sens_dat2$pos_frac <- sens_dat2$positives / sens_dat2$pool.size

# sensitivity vs. fraction of pool positive
ggplot(sens_dat2, aes(x=pos_frac, y=sensitivity, group=as.factor(pool.size))) +
  geom_line(aes(color=as.factor(pool.size))) + 
  guides(color=guide_legend(title="Pool Size")) + 
  geom_hline(yintercept = sens_dat2$sensitivity[sens_dat2$pool.size==1]) + 
  theme(legend.position="bottom") + 
  xlab("Number Positive / Pool size") + ylab("Sensitivity") 

ggplot(sens_dat2, aes(x=ct_avg, y=sensitivity, group=as.factor(pool.size))) +
  geom_line(aes(color=as.factor(pool.size))) + 
  guides(color=guide_legend(title="Pool Size")) + 
  geom_hline(yintercept = sens_dat2$sensitivity[sens_dat2$pool.size==1]) + 
  theme(legend.position="bottom") + 
...
TODO: plot (1) sensitivity as a function of Ct values ; (2) sensitivty as function of number of infected
...
sapply(1:B, function(b){
            sum(temp[2:(N+1), b] * sens)/sum(temp[2:(N+1), b])}) #sensitivity 

sapply(1:B, function(b){
            (sum(temp[2:(N+1), b] * sens)/sum(temp[2:(N+1), b]))/sens_fxn(1)}) #ppa

(1/N) + sapply(1:B, function(b){sum(sens*temp[1:(N+1), b])}) 

(1/N)*(1 + (sens[1]*sum(temp[2:(N+1),1]))) 
